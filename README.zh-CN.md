...
| ---------------- | --------------------------------------------------------------------------------------------- | ------------------------------------ |
| 确定性控制反转   | CLI 持有交付主循环，规划、开发、评审和验收 Agent 都是有终点的任务执行者                       | 防止监督 Agent 跑偏、遗忘或提前退出  |
| 契约化计划流水线 | Agent 动态产出设计方案 → 验收文档 → manifest DAG，阶段之间有 machine gate 和 review gate    | 防止需求、设计、拆解和实现各说各话   |
...
```text
需求 → Agent 动态形成设计 / 验收 / DAG → schema + lint + review → 确定性 Loop → 开发 / 评审 / 合并 / 最终验收
```
...
|                          | Computational：确定性、快速、可重复                                                                                                                                                                                          | Inferential：理解语义、判断质量                                                                                                                                                                    |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Guide：行动前**  | • 计划校验：schema 与 lint 拦截无效 DAG。• DAG 调度：依赖、ready nodes（`ready_nodes`）和并发上限控制执行顺序。• 任务合同：contract 与 authority 明确目标、边界和权限。• 执行协议：实例事实和精确命令避免 Agent 猜测。 | • 设计方案：统一架构边界和技术口径。• 验收定义：把目标转成可执行的用户 flow。• 仓库规则：`AGENTS.md` 保存长期工程约束。• 角色上下文：Role Guide、Skills 和 `guide_refs` 提供最小必要知识。 |
| **Sensor：行动后** | • 结构化证据：记录验证结果和交付引用。• 自动检查：lint、tests 和 CI 验证实现。• 集成状态：检查 Pull Request、目标分支和 merge 状态。• 状态对齐：reconcile manifest、平台与 Git，支持中断续跑。                           | • 计划评审：检查设计、验收和 DAG 是否一致。• Reviewer：判断边界、风险、兼容性和实现质量。• Acceptor：按用户 flow 做最终验收。• 反馈结论：将 verdict 和失败原因交给 Loop。                      |
