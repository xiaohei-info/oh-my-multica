...
- 项目级语言配置：`omac init` 选择 `en` 或 `cn` 并写入 `.omac/config.yaml`；没有该字段的项目默认英文。
- `work show` 的协议、权威顺序和 table 视图、Guide 加载以及顶层 CLI help 支持中英文；JSON schema、任务事实和可执行 submit 命令保持不变。
- 全部内置 OMAC Guide 增加英文镜像；中文继续通过同一项目配置读取。
- README 改为英文主文档，并保留简体中文镜像。
