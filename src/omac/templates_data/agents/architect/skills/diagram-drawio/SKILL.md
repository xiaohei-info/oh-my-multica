...
When generating diagrams destined for an Obsidian vault note, follow this embed pattern:

1. **Co-locate files** — copy both `.drawio` and exported `.png` to the **same directory** as the target note. This makes the `![[file.png]]` wikilink resolve correctly and keeps the editable source alongside the rendered image.
2. **Use wikilink embed** — insert `![[filename.png]]` (not markdown `![](path)`) into the note. Obsidian resolves wikilinks against the vault root, and the embed renders inline.
3. **Add a legend section** below the embed — include a bullet list mapping colors to cloud platforms / tiers / domains, and distinguish solid vs dashed arrow semantics. This makes the diagram self-documenting when viewed in Obsidian without draw.io.
4. **Reference the source section** — if the diagram replaces an existing one (e.g. a Mermaid block in §5.2), add a blockquote noting the replacement and where the original lives.
5. **Keep the `.drawio` source** — do not delete it after export. The user or a future session may need to iterate on the diagram.

For detailed patterns on extracting architecture from design documents and cloud-platform color conventions, see `references/obsidian-integration.md`.
