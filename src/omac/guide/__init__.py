...
def _read_markdown(path: PurePosixPath) -> str:
...
def load_topic(name: str) -> str:
    ...
    return _read_markdown(PurePosixPath(f"{name}.md"))


def load_role_topic(name: str) -> str:
    if name not in ROLE_TOPICS:
        raise ValueError(f"unknown guide role topic: {name}")
    return _read_markdown(PurePosixPath("roles") / f"{name}.md")


def load_artifact_topic(name: str) -> str:
    if name not in ARTIFACT_TOPICS:
        raise ValueError(f"unknown guide artifact topic: {name}")
    return _read_markdown(PurePosixPath("artifacts") / f"{name}.md")
