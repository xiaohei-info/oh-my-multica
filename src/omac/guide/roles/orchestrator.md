...
- 验收文档中每个 flow、稳定步骤 ID 与显式 `action.kind`。只为 `business-action` 分配实现归属。
...
- 验收责任是显式结构，不依赖 Action 前缀或节点名字：machine gate 会检查每个 flow 的唯一完整 owner、
...
8. 先根据权威 acceptance 内容建立全局责任矩阵，而不是按节点名字猜测：
   `acceptance_claims` 只给执行完整 flow 的唯一集成/closeout 节点，且不复制 Action ID；
   `acceptance_contributions` 为组件节点分配精确 `flow_id` 与 `business-action` IDs；
...
- 每个 flow 恰有一个完整 owner，每个业务 Action 至少有一个 contribution owner，且完整 owner 等于或传递依赖全部贡献节点；没有缺失、重复或上游节点冒充完整 flow 的 claim。
...
- 禁止创建“先搭骨架”“暂时返回假数据”或必须由后续节点补齐才能成立的节点；测试替身不得成为生产假数据兜底。
