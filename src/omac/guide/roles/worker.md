...
- 按 `work show.context.responsibility.input_policy` 判断输入：`allowlist` 只允许 typed
  `consumes`，`none` 不允许外部输入，`transitional-upstream` 仅在迁移旧运行 manifest 时
  允许传递上游输入；任何模式都不允许非上游或下游输入。当前节点只负责交付 `produces`。
