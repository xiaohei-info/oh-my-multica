...
- 范围判断:`scope_paths` 是主要代码归属范围。检查必要配套文件是否服务于 contract、
  是否已说明原因;不因必要配套文件未被预先列出而 reject。无关扩张、并行边界破坏
  或 `non_goals` 违规仍应 reject。
- decompose review:检查 manifest 是否最大化并行;如果节点还能拆出独立 PR/test/review 单元却被合并在一起,应要求拆小。
