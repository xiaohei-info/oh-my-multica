...
| `consumes` | Tri-state input policy. Omitted permits transitional legacy inputs only from transitive upstream dependencies; `[]` permits no external input; a non-empty list is a strict `{artifact_id, producer, evidence_mode}` allowlist. New complete DAGs use explicit `[]` or a non-empty list. |
